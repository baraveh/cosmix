...
unsigned char* try_evict_page(item_t* pce)
{
    ...
    unsigned char* epc_page_ptr = (unsigned char*)(g_pfcounter_base_page_cache_ptr + (pce->epc_page_index * PFCOUNTER_PAGE_SIZE));
...
int pfcounter_mstore_init(void* priv_data){
    ...
    g_page_table = new PageTable(/*num_of_buckets=*/(PFCOUNTER_PAGE_CACHE_SIZE/PFCOUNTER_PAGE_SIZE) * 10,/*backing_store_size=*/num_bs_entries, /*page cache size=*/ (PFCOUNTER_PAGE_CACHE_SIZE/PFCOUNTER_PAGE_SIZE));

    init_page_cache(&g_pfcounter_page_cache, &g_pfcounter_base_page_cache_ptr, PFCOUNTER_PAGE_CACHE_SIZE, PFCOUNTER_PAGE_SIZE);
...
void* pfcounter_mpf_handler_c(void* bs_page){
    ...
    if (!g_page_table->try_add(bs_page_index, free_epc_page_index, dirty))
    {
        ...
        free_epc_ptr = (unsigned char*)(g_pfcounter_base_page_cache_ptr + (found->epc_page_index * PFCOUNTER_PAGE_SIZE));
    }
